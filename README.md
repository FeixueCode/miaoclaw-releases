...
**下载最新版本 → [Releases](https://github.com/FeixueCode/miaoclaw-releases/releases/latest)** ｜ 最新：**v2.1.5**
...
- **收尾更诚实**：多步骤任务时间不够没做完时，喵喵现在会如实给出「已完成 / 未完成」清单，回复「继续」就能接着做完剩下的——不再含糊收尾…
- **反应更轻快**：与当前任务关系不大的扩展说明不再整篇塞给模型（曾经一次多占上万字的空间），改为一行简介、需要时再展开——回答更专注、上下…
- **界面小改进**：设置 → Agent 角色：「新建自定义 Agent」表单默认收起，页面更清爽；点「＋ 新建」或编辑某个 Agent 时…

完整更新说明见 [Releases](https://github.com/FeixueCode/miaoclaw-releases/releases/tag/v2.1.5)。

---

## English

**Miaoclaw** is a local-first AI assistant for everyday work, delivered as a Windows desktop app with a Chinese-first UX. Latest: **v2.1.5**.
